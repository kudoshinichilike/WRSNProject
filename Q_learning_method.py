...
def get_charging_time(network=None, q_learning=None, state=None, alpha=0):
    ...
    #     d = distance.euclidean(q_learning.action_list[state], node.location)
    #     p = para.alpha / (d + para.beta) ** 2
    #     if node.energy - time_move * node.average_used < node.energy_thresh and p - node.average_used > 0:
    #         s1.append((node.id, p))
    #     if node.energy - time_move * node.average_used > node.energy_thresh and p - node.average_used < 0:
    #         s2.append((node.id, p))
    # t = []
    #
    # for index, p in s1:
    #     t.append((network.node[index].energy_thresh - network.node[index].energy + time_move * network.node[
    #         index].average_used) / (

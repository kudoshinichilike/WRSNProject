...
class Q_LearningSensor:
    def __init__(self, sensor, init_q_table_func=init_q_table_function, init_state_func=calc_state_function, alpha=0.5,
                 gamma=0.5, nb_action_mc=81, mc_position=81):
        self.sensor = sensor
        self.alpha = alpha  # learning rate
        self.gamma = gamma  # scale factor
        self.state = init_state_func(self.sensor, nb_action_mc=nb_action_mc-1)  # current state
        self.action = 0  # current action
        self.q_table = init_q_table_func(sensor, nb_action_mc=nb_action_mc)  # q table

    def update(self, network, reward_func=reward_function, mc_position=81):
        """
        ...
        """
        self.state = calc_state_function(self.sensor, mc_position)
    ...
    def set_reward(self, reward_func=reward_function, network=None):
        ...
        new_value_q = (1 - self.alpha) * self.q_table[self.state[0]][self.state[1]][self.state[2]][self.action] + self.alpha * (reward + self.gamma * self.q_max(state))
    ...
    def q_max(self, state):
        ...
        :return: q_max
        """
        return np.amax(self.q_table[state[0]][state[1]][self.state[2]])
